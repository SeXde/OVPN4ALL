<script lang="ts">
	...
	async function sendData() {
		...
		postError = null;
		await fetch('http://localhost:8082/api/users', options)
		.then(res => res.json())
		.then(res => postError = res.error)
		.catch(() => {postError = "Server request failed"});
		isLoading = false;
		if (postError) return;
        goto("/users");
    ...
    <form on:change={checkDefault} on:submit|preventDefault={sendData} class= "px-8 pt-6 pb-8 mb-4">
		<div class="flex flex-col items-center">
			<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
			  ...
			  <h2 class="text-center text-2xl"> Register form</h2>
		</div>
		...
		<div class="relative my-3">
			<label for="default-toggle" class="inline-flex relative items-center cursor-pointer">
				<input on:click={() => isAdmin = !isAdmin} type="checkbox" value="" id="default-toggle" class="sr-only peer">
        ...
        {:else}
            <button disabled type="submit" class="my-3 disabled:opacity-25 mx-auto mt-5 w-36 py-2 flex justify-center text-light rounded-lg border-2 border-light hover:text-primary hover:border-primary disabled:border-stone-500 disabled:text-stone-500 font-semibold transition-colors">Register user</button>
        {/if}
        {#if postError}
