<script lang="ts">
	...
	const selectPage = (index: number): void => {
		...
		console.log("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA: ", selectedPage);
	}
...
<Header navbar={true}/>
	...
	<div class="flex flex-row m-5">
		...
		{/if}
		{#if selectedPage[1]}
			<div class="mx-auto text-green-500 bg-light_dark w-3/5 p-14 rounded-lg">
				<code>{createServerConfigLog.content}</code>
			</div>
		{/if}
		{#if selectedPage[2]}
		<div class="mx-auto text-green-500 bg-light_dark w-3/5 p-14 rounded-lg">
			<code>{createUserCertLog.content}</code>
		</div>
		{/if}
		{#if selectedPage[3]}
		<div class="mx-auto text-green-500 bg-light_dark w-3/5 p-14 rounded-lg">
			<code>{createUserVPNFileLog.content}</code>
		</div>
		{/if}
		{#if selectedPage[4]}
		<div class="mx-auto text-green-500 bg-light_dark w-3/5 p-14 rounded-lg">
			<code>{deleteUserLog.content}</code>
		</div>
		{/if}
		{#if selectedPage[5]}
		<div class="mx-auto text-green-500 bg-light_dark w-3/5 p-14 rounded-lg">
			<code>{OVPNLog.content}</code>
		</div>
		{/if}
